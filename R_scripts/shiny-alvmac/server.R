...
# Expression set
exprSet = readRDS(file = 'data/alvmac.eSet.rds')

# GOexpress result object
gox.pvalue = readRDS(file = 'data/gox.pvalue.rds')
...
# Define the reactive components of the application -----------------------
...
shinyServer(
  function(input, output) {
    ...
    output$GOscores <- renderDataTable(
      gox.pvalue$GO[which(gox.pvalue$GO$total_count >= input$min.total),],
      options = list(
        pageLength = 20)
    )
